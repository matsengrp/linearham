...
assert env.GetOption("help") or options["run_partis"] != options["run_linearham"], \
    "Exactly one of '--run-partis' and '--run-linearham' must be specified"
assert env.GetOption("help") or options["run_linearham"] or options["fasta_path"] is not None, \
    "Please specify the '--fasta-path' argument"
assert env.GetOption("help") or options["run_partis"] or options["template_path"] is not None, \
    "Please specify the '--template-path' argument"
...
if options["run_partis"]:
    ...
    def partis_build(outdir, c):
        return env.Command(["lib/partis/packages/samtools/samtools",
