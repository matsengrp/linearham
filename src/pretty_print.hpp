...
namespace std {
// Pre-declarations of container types so we don't actually have to include the
// relevant headers if not needed, speeding up compilation time.
...
// SFINAE type trait to detect a container based on whether T::const_iterator
// exists.
...
template <typename T>
struct is_container
    : public ::std::integral_constant<bool, is_container_helper<T>::value> {};
...
template <typename T, typename TTraits, typename TAllocator>
const delimiters_values<char> delimiters<::std::set<T, TTraits, TAllocator>,
                                         char>::values = {"{", ", ", "}"};
template <typename T, typename TTraits, typename TAllocator>
...
const delimiters_values<wchar_t>
    delimiters<::std::set<T, TTraits, TAllocator>, wchar_t>::values = {
        L"{", L", ", L"}"};
...
struct print_container_helper {
  ...
  inline void operator()(ostream_type &stream) const {
    ...
    for (typename T::const_iterator beg = _container.begin(),
                                    end = _container.end(), it = beg;
         it != end; ++it) {
...
struct custom_delims_wrapper : public custom_delims_base {
  ...
  ::std::ostream &stream(::std::ostream &stream) {
    return stream << ::pretty_print::print_container_helper<
...
inline basic_ostream<TChar, TCharTraits> &operator<<(
    basic_ostream<TChar, TCharTraits> &stream, const tuple<Args...> &value) {
  ...
  ::pretty_print::pretty_tuple_helper<const tuple<Args...> &, sizeof...(Args),
                                      TChar, TCharTraits>::print(stream, value);
