...
def parse_cluster_annotation(annotation_list, cpath, partition_index, cluster_index, seed_unique_id):
    if len(annotation_list) == 1:
        print ' only one annotation in partis output file. Using it.'
        return annotation_list[0]
    if cpath is None:
        raise Exception('  no cluster path in input file containing multiple clusters. Need cluster path to use arguments to parse the desired cluster from the partis output file')
    else:
        ipartition = cpath.i_best if partition_index is None else partition_index
        print '  found %d clusters in %s' % (len(cpath.partitions[ipartition]), 'best partition' if partition_index is None else 'partition at index %d (of %d)' % (ipartition, len(cpath.partitions)))
        ...
        if seed_unique_id is not None:
            clusters_to_use = [c for c in clusters_to_use if seed_unique_id in c]  # NOTE can result in more than one cluster with the seed sequence (e.g. if this file contains intermediate annotations from seed partitioning))
            print '    removing clusters not containing sequence \'%s\' (leaving %d)' % (seed_unique_id, len(clusters_to_use))
            if len(clusters_to_use) > 1:
                raise Exception('Multiple clusters with partis seed sequence with uid \'%s\'. Partis output file may contain intermediate annotations from seed partitioning. Try using --cluster-index to choose one.' % seed_unique_id)
    
    if len(clusters_to_use) != 1:
        raise Exception('Options passed must uniquely identify 1 cluster in the partis output file but instead resulted in %d clusters.' % len(clusters_to_use))
    ...
    return annotations[clusters_to_use[0]]
...
def cluster_sequences(annotation, use_indel_reversed_sequences=False):
    ...
    #TODO replace 'seqs' with something more transparent like was originally being used:
    ...
    seqfos += [{'name' : u, 'seq' : s} for u, s in zip(annotation['unique_ids'], annotation['seqs' if use_indel_reversed_sequences else 'input_seqs'])]
    return seqfos
...
def write_cluster_fasta(fname, seqfos):
    ...
    write_to_fasta(OrderedDict(seqfos), fname)

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Parse the sequences in each clonal family.")
    parser.add_argument("partis_yaml_file", type=str, help="Path to partis output YAML file.")
    parser.add_argument('--fasta-output-file', type=str, required=True, help='output fasta file name')
    parser.add_argument('--yaml-output-file', type=str, required=True, help='output yaml file name')
    ...
    cluster_annotation = parse_cluster_annotation(annotation_list, cpath, args.partition_index, args.cluster_index, args.seed_unique_id)
    warn_duplicate_naives(cluster_annotation)
    #write yaml
    utils.write_annotations(args.yaml_output_file, glfo, [cluster_annotation], set(cluster_annotation))
    #write fasta
    seqfos = cluster_sequences(cluster_annotation, use_indel_reversed_sequences=args.indel_reversed_seqs)
